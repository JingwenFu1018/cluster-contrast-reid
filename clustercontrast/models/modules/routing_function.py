...
class RountingFunction(nn.Module):
    ...
    def __init__(self, in_channels, kernel_number, dropout_rate=0.2, proportion=40.0):
        ...
        self.fc_scale = nn.Linear(in_channels, kernel_number, bias=False)       
        ...
        trunc_normal_(self.fc_scale.weight, std=.02)
    ...
    def forward(self, x):
        ...
        scales = self.dropout3(x)
        scales = self.fc_scale(scales)
        scales = torch.sigmoid(scales)
        scales = scales + 0.5

        return alphas, angles, scales
