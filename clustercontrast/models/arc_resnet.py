...
from .modules import AdaptiveRotatedConv2d, RountingFunction, AdaptiveRotatedScaledConv2d
import os.path as osp


__all__ = ['ARCResNet', 'arcresnet18', 'arcresnet34', 'arcresnet50', 'arcresnet101',
           'arcresnet152']
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if str(self.n_block) not in self.replace:
            ...
            self.conv2 = AdaptiveRotatedScaledConv2d(
                in_channels=width,
                out_channels=width,
                kernel_size=3, 
                stride=stride,
...
class ARCResNetRaw(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, n_stage=None, replace=None, kernel_number=None):
        ...
        for idx_block in range(1, blocks):
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load(base_dir + '/checkpoint-best.pth', weights_only=False)
